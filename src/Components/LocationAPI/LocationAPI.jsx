  ...
  const getLocationData = async () => {
    try {
      ...
      const weather = await axios.get(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=${weatherAPI}`
      );
      ...
      // Trails API
      const trailsAPI = await axios.get(
        `https://www.hikingproject.com/data/get-trails?lat=${lat}&lon=${long}&maxDistance=10&key=${apiTrailsKey}`
      );
  ...
  const handleSubmit = (e) => {
    ...
    (async () => {
      try {
        ...
        const userInput = await axios.get(
          `https://api.opencagedata.com/geocode/v1/json?q=${cityInput}%20${stateInput}&key=${apiOpenCageKey}`
        );
        ...
        const weather = await axios.get(
          `http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=${weatherAPI}`
        );
  ...
  return (
    <>
      ...
      <form action="POST" className="search-container" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="userSearchInput button" value="Search">
          Search
        </button>
      </form>
      ...
      {isLoading && (
        ...
        >
          <div id="area-container" className="area-container">
            <h2>
              We have found {trailsData.length} trails around the area of:
            </h2>
            <div className="city-state">
              <h2 className="city-text-location">
                {userLocation.city ? userLocation.city : cityInputCaps}
              </h2>
              <h2 className="region-text-location">
                {userLocation.region ? userLocation.region : stateInputCaps}
              </h2>
            </div>
          </div>
          ...
          <GetWeatherLocation
            ...
            userWeatherMain={userWeatherMain}
